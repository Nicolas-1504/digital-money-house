...
import { transaction } from "DMH/services/transactions.service";
...
import CardListSelectItem from "../addMoneyMethods/card/CardListSelectItem";
...
const SelectCardService = () => {
  ...
  const onSubmit = () => {
    ...
    transaction({
      ...
      if (res?.id) {
        ...
        router.push("/pay-services/error-pay");
      }
    });
  };
  ...
  useEffect(() => {
    setAmount(Math.ceil(Math.random() * (5000 - 1000) + 1000));
    getCards()
